...
def save_images(images, path, dim=(250,250, 3)):
    ...
    for i in range(images.shape[1]):
        name = ['class0_RGB_rd_1e-8.png', 'class1_RGB_rd_1e-8.png']
        image = np.asarray(tf.reshape(images[:,i], dim).eval())
        print (image.shape)
        scipy.misc.imsave(os.path.join(path, name[i]), image)
    """
    name = ['w0_minus_w1_RGB.png', 'w0_plus_w1_RGB.png']
    image_minus = np.asarray(tf.reshape(images[:,0] - images[:,1], dim).eval())
    image_plus = np.asarray(tf.reshape(images[:,0] + images[:,1], dim).eval())
    scipy.misc.imsave(os.path.join(path, name[0]), image_minus)
    scipy.misc.imsave(os.path.join(path, name[1]), image_plus)
...
def main():
    ...
    with tf.Session() as sess:
        ...
        #dec_b = tf.get_collection(tf.GraphKeys.VARIABLES,"dec_b")[0]
